...
export function Sidebar({ outletName, onNavigate }: { outletName?: string; onNavigate?: () => void }) {
  ...
  return (
    <nav aria-label="Navigasi utama" className="flex h-full flex-col bg-gray-900 text-gray-200">
      <div className="border-b border-gray-800 px-4 py-4">
        <p className="text-lg font-bold text-white">GawEEE</p>
        {outletName && <p className="truncate text-xs text-gray-400">{outletName}</p>}
      </div>
                  ...
                  {item.children.map((child) => (
                    <Link
                      ...
                      className={`block rounded px-2 py-1 text-sm ${
                        pathname === child.href ? 'text-blue-400' : 'text-gray-400 hover:text-gray-200'
                      }`}
                    >
                      {child.label}
                    </Link>
      ...
      <div className="border-t border-gray-800 px-4 py-3">
        <a href="/dashboard/help" className="block text-sm text-gray-400 hover:text-gray-200">
          ❓ Help &amp; Support
        </a>
      </div>
    </nav>
