...
export function BookingsManager({ outletId }: { outletId: string }) {
  ...
  const load = useCallback(async () => {
    setIsLoading(true)
    const [bookingsRes, staffRes, facilitiesRes] = await Promise.all([
      fetch(`/api/bookings?outlet_id=${outletId}`),
      fetch(`/api/staff?outlet_id=${outletId}`),
      fetch(`/api/facilities?outlet_id=${outletId}`),
    ])
    ...
    <div className="space-y-4">
      ...
      {showForm && (
        <form onSubmit={handleCreate} className="grid grid-cols-1 gap-3 rounded-lg border border-gray-200 p-4 sm:grid-cols-3">
          ...
          <div className="space-y-1">
            <label className="block text-sm font-medium text-gray-700">Fasilitas (opsional)</label>
            <select
        ...
        <table className="min-w-full divide-y divide-gray-200 text-sm">
          ...
          <tbody className="divide-y divide-gray-100 bg-white">
            {isLoading ? (
              ...
              <tr>
                <td colSpan={8} className="px-4 py-6 text-center text-gray-400">Belum ada booking</td>
              </tr>
            ) : (
              bookings.map((b) => {
                ...
                return (
                  <tr key={b.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-4 py-2 text-gray-700">{b.facilities?.name ?? '-'}</td>
