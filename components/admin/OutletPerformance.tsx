          ...
          <div className="overflow-x-auto">
            <table className="min-w-full divide-y divide-gray-200 text-sm">
              ...
              <tbody className="divide-y divide-gray-100">
                {sorted.map((outlet, i) => (
                  <tr key={outlet.outlet_id} className="cursor-pointer hover:bg-gray-50">
                    <td className="py-2 pr-4 text-gray-400">
                      <Link href={`/dashboard/admin/outlets/${outlet.outlet_id}`} className="block">
                        {i + 1}
                      </Link>
                    </td>
                    <td className="py-2 pr-4 font-medium text-[var(--brand-600)]">
                      <Link href={`/dashboard/admin/outlets/${outlet.outlet_id}`} className="hover:underline">
                        {outlet.outlet_name}
                      </Link>
                    </td>
                    <td className="py-2 pr-4 text-right text-gray-700">
                      <Link href={`/dashboard/admin/outlets/${outlet.outlet_id}`} className="block">
                        {formatCurrency(outlet.revenue_mtd)}
                      </Link>
                    </td>
                    <td className="py-2 pr-4 text-right text-gray-700">
                      <Link href={`/dashboard/admin/outlets/${outlet.outlet_id}`} className="block">
                        {formatPercent(outlet.profit_margin_percent / 100)}
                      </Link>
                    </td>
                    <td className="py-2 pr-4 text-right text-gray-700">
                      <Link href={`/dashboard/admin/outlets/${outlet.outlet_id}`} className="block">
                        {outlet.transaction_count}
                      </Link>
                    </td>
                    <td className="py-2 pr-4 text-right text-gray-700">
                      <Link href={`/dashboard/admin/outlets/${outlet.outlet_id}`} className="block">
