...
export function StockTurnoverReport({ outletId }: { outletId?: string }) {
  ...
  const load = useCallback(async () => {
    setIsLoading(true)
    const res = await fetch(`/api/reports/stock-turnover?outlet_id=${effectiveOutlet}&start=${range.start}&end=${range.end}`)
  ...
  return (
    <div className="space-y-3">
      <div className="flex flex-wrap items-center justify-between gap-2">
        ...
        <ExportCsvButton filename="stock-turnover" rows={rows} />
      </div>
        ...
        <table className="min-w-full divide-y divide-gray-200 text-sm">
          <thead className="bg-gray-50">
            <tr>
              ...
              <th className="px-4 py-2 text-right font-semibold text-gray-600">HPP Terjual</th>
