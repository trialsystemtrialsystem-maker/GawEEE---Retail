...
export function CashierReport({ outletId }: { outletId?: string }) {
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <ExportCsvButton filename={`cashier-report-${range.start}-${range.end}`} rows={rows} />
      </div>
        ...
        <table className="min-w-full divide-y divide-gray-200 text-sm">
          <thead className="bg-gray-50">
            <tr>
              ...
              <th className="px-4 py-2 text-right font-semibold text-gray-600">Transaksi</th>
              <th className="px-4 py-2 text-right font-semibold text-gray-600">Pendapatan</th>
              <th className="px-4 py-2 text-right font-semibold text-gray-600">Rata-rata/Transaksi</th>
          ...
          <tbody className="divide-y divide-gray-100 bg-white">
            {isLoading ? (
              ...
              <tr>
                <td colSpan={5} className="px-4 py-6 text-center text-gray-400">Belum ada transaksi</td>
              </tr>
            ) : (
              rows.map((r) => (
                <tr key={r.cashier_id} className="hover:bg-gray-50">
                  ...
                  <td className="px-4 py-2 text-right text-gray-700">
                    {formatCurrency(r.transactions > 0 ? r.revenue / r.transactions : 0)}
                  </td>
