...
export async function GET(request: NextRequest) {
  ...
  const { data, error } = await auth.supabase.from('coupons').select('*').eq('outlet_id', outletId).order('created_at', { ascending: false })
...
export async function POST(request: NextRequest) {
  ...
  const result = validate(couponSchema, body)
