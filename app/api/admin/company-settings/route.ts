...
type Settings = { receipt_header?: string; receipt_footer?: string; [key: string]: unknown }
...
export async function PATCH(request: NextRequest) {
  ...
  const { name, tax_id, tax_rate, receipt_header, receipt_footer } = result.data
  ...
  const settings = { ...((current?.settings ?? {}) as Settings), receipt_header: receipt_header ?? '', receipt_footer: receipt_footer ?? '' }
  ...
  await auth.supabase.from('audit_log').insert({
    ...
    new_values: { name, tax_id, tax_rate, receipt_header, receipt_footer },
    status: 'success',
  })
