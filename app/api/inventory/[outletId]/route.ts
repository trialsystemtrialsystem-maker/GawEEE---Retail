  ...
  try {
    const rowsRaw = await fetchAllRows<Row>((from, to) => {
      let query = auth.supabase
        .from('inventory')
        .select(
          'product_id, quantity_on_hand, quantity_reserved, quantity_available, alert_status, reorder_level, avg_cost, bin_location, products(name, sku, barcode, category_id, purchase_price, selling_price, unit_type, reorder_level, reorder_quantity, supplier_id, is_active, product_categories(name), suppliers(name))'
        )
    ...
    const base = rows.map((r) => {
      ...
      return {
        ...
        avg_cost: r.avg_cost,
        // Value at the moving-average cost (what COGS uses) once one exists,
        // else the master purchase price; the master-price value stays visible.
        cost_value: (r.avg_cost ?? p?.purchase_price ?? 0) * r.quantity_on_hand,
        master_cost_value: (p?.purchase_price ?? 0) * r.quantity_on_hand,
